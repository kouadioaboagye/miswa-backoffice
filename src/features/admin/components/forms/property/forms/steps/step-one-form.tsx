import { useListMunicipalitiesQuery } from '@/lib/data-service/general/general.hooks';
...
function StepOneForm({ form }: Readonly<StepOneFormProps>) {
    ...
    const { data: municipalitiesResponse, isLoading: isMunicipalitiesLoading } =
        useListMunicipalitiesQuery();
    const { data: municipalities = [] } = municipalitiesResponse || {};

    const { data: businessesResponse, isLoading: isBusinessesLoading } =
        useListOwnersQuery();
    const { data: businesses = [] } = businessesResponse || {};

    const { data: buildingResponse, isLoading: isBuildingLoading } =
    ...
    React.useEffect(() => {
        if (selectedBuildingId) {
            ...
            const selectedBuilding = buildings.find(
                (building) => building.id.toString() === selectedBuildingId
            );
            ...
            if (selectedBuilding) {
                ...
                if (selectedBuilding.id_business) {
                    ...
                    console.warn(
                        "Le bâtiment sélectionné n'a pas de id_business"
                    );
                ...
                if (selectedBuilding.id_municipality) {
                    ...
                    console.warn(
                        "Le bâtiment sélectionné n'a pas de id_municipality"
                    );
                    setValue('ville', '', {
                        shouldValidate: true,
                        shouldDirty: true
                ...
                console.warn(
                    "Aucun bâtiment trouvé avec l'ID:",
                    selectedBuildingId
                );
            ...
            if (
                getValues('proprietaire') &&
                !getValues('proprietaire').includes('manual-')
            ) {
                setValue('proprietaire', '', { shouldValidate: true });
            }
    ...
    const correspondingOwner =
        selectedBuilding && selectedBuilding.id_business
            ? businesses.find(
                  (business) =>
    ...
    const correspondingMunicipality =
        selectedBuilding && selectedBuilding.id_municipality
            ? municipalities.find(
                  (municipality) =>
        ...
        <div>
            <section>
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                    <div>
                        <Label htmlFor="typebatiment" isRequired>
                            Selectionnez le batiment
                        </Label>
                        <Controller
                            name="typebatiment"
                            control={control}
                        ...
                        />
                        <InputErrorMessage
                            message={errors.typebatiment?.message}
                        />
                    </div>

                    <div>
                        <Label htmlFor="nom" isRequired>
                            Nom du bien
                        </Label>
                        <Input
                            id="nom"
                ...
                <div>
                    ...
                    <div className="relative">
                                    ...
                                    {selectedBuildingId &&
                                        !correspondingOwner && (
                                            <p className="text-xs text-yellow-600 mt-1">
                                                Aucun propriétaire associé à ce
                                                bâtiment. Veuillez en
                        ...
                        />
                        <InputErrorMessage
                            message={errors.proprietaire?.message}
                        />
                    </div>
                </div>
            </section>
            ...
            <section className="mt-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                            ...
                            render={({ field, fieldState: { error } }) => (
                                <div>
                                    ...
                                    {selectedBuildingId &&
                                        !correspondingMunicipality && (
                                            <p className="text-xs text-yellow-600 mt-1">
                                                Aucune municipalité associée à
                                                ce bâtiment. Veuillez en
                                                sélectionner une manuellement.
                                            </p>
                                        )}
                                    <InputErrorMessage
                                        message={error?.message}
                    ...
                    <div>
                        <Label htmlFor="quartier" isRequired>
                            Quartier
                        </Label>
                        <Input
                            id="quartier"
                    ...
                    <div>
                        <Label htmlFor="adresse" isRequired>
                            Adresse complète
                        </Label>
                        <Input
                            id="adresse"
