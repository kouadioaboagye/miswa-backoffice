...
import { useListComoditiesQuery } from '@/lib/data-service/property/property.hooks';
        ...
        <div>
            <section>
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                    <div>
                        <Label htmlFor="annee" isRequired>
                    ...
                    <div>
                        <Label htmlFor="superficie" isRequired>
                            Superficie totale (m²)
                        </Label>
                        <Input
                            id="superficie"
                            ...
                            type="number"
                            className={
                                errors.superficie ? 'border-red-500' : ''
                            }
                        />
                        <InputErrorMessage
                            message={errors.superficie?.message}
                        />
                    </div>

                    <div>
                        <Label htmlFor="rooms_count" isRequired>
                            Nombre de chambres
                        </Label>
                        <Input
                            id="rooms_count"
                            ...
                            type="number"
                            className={
                                errors.rooms_count ? 'border-red-500' : ''
                            }
                        />
                        <InputErrorMessage
                            message={errors.rooms_count?.message}
                        />
                    </div>
                </div>
            </section>
                                ...
                                {!isLoading && !error && (
                                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 border border-gray-200 rounded-lg">
                                        {commodities.length === 0 ? (
                                            ...
                                            commodities.map((commodite) => (
                                                <div
                                                    key={commodite.id}
                                                    className={`flex items-center space-x-3 p-3 border rounded-lg transition-colors ${
                                                        field.value?.includes(
                                                            commodite.id.toString()
                                                        )
