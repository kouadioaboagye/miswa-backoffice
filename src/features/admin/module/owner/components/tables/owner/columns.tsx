...
const OwnerActions = ({ owner, onDetails, onEdit, onDelete }:
  {
    owner: Owner,
  ...
  return (
    <>
      <div className="flex items-center gap-6">
        <button className="flex size-12 items-center justify-center rounded-full bg-[#1EA64A]/10 hover:bg-[#1EA64A]/20 transition-colors" onClick={()=> onDetails(owner.id.toString())}>
          <EyeIcon2 />
        </button>
        <button className="flex size-12 items-center justify-center rounded-full bg-[#5D5FEF]/10 hover:bg-[#5D5FEF]/20 transition-colors" onClick={()=> onEdit(owner.id.toString())}>
          <EditIcon />
        </button>
        <button
...
export const columns = (
  ...
  onDelete: (id: string) => Promise<void>
    ...
    },
    {
      ...
      cell: ({ row }) => <OwnerActions owner={row.original} onDelete={onDelete} onDetails={onDetails} onEdit={onEdit}/>,
    },
  ];
