...
import { Plus, UsersIcon, UserCheckIcon, BuildingIcon, PhoneIcon, MailIcon } from 'lucide-react';
...
const ListOwnerView = () => {
  ...
  const { data: response, isLoading, error, refetch, isRefetching } = useListOwnersQuery(currentPage, pageSize);
      ...
      <DataTableLayout
        ...
        action={{
          refresh: (
            ...
            >
              <RefreshIcon /> <span className="text-[1.3rem]">RAFRAICHIR</span>
            </Button>
          ),
          add: (
            <Button
            ...
            >
              <Plus /> <span className="text-[1.3rem]">NOUVEAU PROPRIÉTAIRE</span>
            </Button>
          ),
        }}
      >
