...
function ListBuildingView() {
    ...
    const {
        ...
        refetch,
        isRefetching
    } = useListBuildingsQuery(currentPage, pageSize);
    ...
    const privateBuildings = buildings.filter(
        (building) => !building.is_public
    ).length;
    ...
    const buildingsWithOwner = buildings.filter(
        (building) => building.business?.owner
    ).length;
